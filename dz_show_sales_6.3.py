...
parser.add_argument('--otkuda', type=int)
parser.add_argument('--dokuda', type=int)
args = parser.parse_args()

if args.otkuda is None and args.dokuda is None:
    with open('sale_list.txt', 'r', encoding='UTF-8') as f:
        a = f.read()
        print(a)
elif args.dokuda is None:
    with open('sale_list.txt', 'r', encoding='UTF-8') as f:
        a = f.read().splitlines()
        for i in a[args.otkuda - 1:]:
            print(i)
else:
    with open('sale_list.txt', 'r', encoding='UTF-8') as f:
        a = f.read().splitlines()
        for i in a[args.otkuda - 1:args.dokuda]:
            print(i)
